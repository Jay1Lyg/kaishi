...
var createActorTemplateInUser = function(obj,callback){
  console.log(obj);
  ActorTemplate.model.findOne()
   ...
   .exec(function(err,ret){
     if(err){
      ...
      .where('actorTemplate_id',ret._id)
               ...
               ActorTemplateInUser.model.create({
                 ...
                 user_id : obj.user_id,
                  ...
                  if(err){
                    ...
                    }else if(obj.name == '时尚达人'){
                      ret1.ifUsed = 2;//未使用
                      ret1.ifCanBeUsed = false;//默认不可用,需要付费
                      ret1.save(function(err){
                        if(err){
                          throw new Error(err);
                        }else{
...
var getActorTemplateStatus = function(result,callback){
   var info = {};
   console.log(result);
   async.eachSeries(result,function(item,next){
     var data = {};
     data.actorTemplateInUser_id = item._id;
     data.actorTemplate_id = item.actorTemplate_id;
     data.user_id = item.user_id;
     data.ifCanBeUsed = item.ifCanBeUsed;
     data.ifUsed = item.ifUsed;
     ActorTemplate.model.findOne()
     .where('_id',item.actorTemplate_id)
     .exec(function(err,ret){
       if(err){
         throw new Error(err);
       }else{
         if(ret.name == '青春绿荫'){
            ...
            info.Greenshadeofyouth = data;
            next();
         }else if(ret.name == '大气蓝调'){
            data.actorTemplateName = ret.name;
            info.AtmosphericBlues = data;
            next();
         }else if(ret.name == '时尚达人'){
            data.actorTemplateName = ret.name;
            info.FashionInsider = data;
            next();
         }
       }
     })
...
module.exports = {
  ...
  createActorTemplateInUser : createActorTemplateInUser,
  getActorTemplateStatus : getActorTemplateStatus
}